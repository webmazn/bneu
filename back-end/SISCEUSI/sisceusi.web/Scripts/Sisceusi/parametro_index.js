$(document).ready(() => {
    ...
    //cargarDesplegables();
    //$('#btn-buscar')[0].click();
    cargarDatosTabla(listaParametro)
});
...
var filtroGeneral = (e) => {
    ...
    let url = `${baseUrl}Parametro/filtroGeneral?${queryParams}`;
...
var filtroAvanzado = (e) => {
    ...
    let parentParametro = $('#parent').val();
    ...
    let columna = $("#column").val();
    let orden = $("#order").val();
    let params = { parentParametro, parametro, idTipoParametro, fechaInicio, fechaFin, estado, registros, pagina, columna, orden };
    let queryParams = Object.keys(params).map(x => params[x] == null ? x : `${x}=${params[x]}`).join('&');

    let url = `${baseUrl}Parametro/filtroAvanzado?${queryParams}`;
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    let method = 'GET';
...
var renderizar = (data, numberCellHeader, pagina, registros) => {
    ...
    if (doRenderizar) {
        content = data.map((x, i) => {
            ...
            let colCodigo = `<td class="text-center" data-encabezado="Código">PAR${pad(x.idParametro, 4)}</td>`;
            let colParametro = `<td data-encabezado="Nombre parámetro">${x.parametro}</td>`;
...
var exportarGeneral = () => {
    ...
    let queryParams = Object.keys(params).map(x => params[x] == null ? x : `${x}=${params[x]}`).join('&')
    let url = `${baseUrl}Parametro/exportarGeneral?${queryParams}`
    location.href = url
}
...
var exportarAvanzado = () => {
    ...
    let params = { parentParametro, parametro, idTipoParametro, fechaInicio, fechaFin, estado, registros, pagina, columna, orden };
    let queryParams = Object.keys(params).map(x => params[x] == null ? x : `${x}=${params[x]}`).join('&');
    let url = `${baseUrl}Parametro/exportarAvanzado?${queryParams}`
    location.href = url;
}
...
var actualizar = (obj) => {
    let id = $(obj).data('id')
    location.href = `${baseUrl}Parametro/NuevoParametro/${id}`
}
...
var deshabilitarRegistro = () => {
    let url = `${baseUrl}Parametro/eliminar?idParametro=${idEliminar}`;
    fetch(url)
    .then(r => r.json())
    .then(j => {
        if (j.success) {

...
var renderizar = (data, numberCellHeader, pagina, registros) => {
    ...
    if (doRenderizar) {
        content = data.map((x, i) => {
            ...
            let btnPlanta = `<a class="btn btn-sm btn-warning text-white btn-table mx-1" href="${baseUrl}PlantaEmpresa/index/${x.idEmpresaIndustria}" data-toggle="tooltip" data-placement="top" title="Mantenimiento de plantas"><i class="fa fa-industry"></i></a>`;
            let btnEliminar = `<div class="btn btn-sm btn-danger btn-table btn-delete mx-1" data-id="${x.idEmpresaIndustria}"><i class="fa fa-trash"></i></div>`;
            let btnEditar = `<div class="btn btn-sm btn-info btn-table btn-edit mx-1" data-id="${x.idEmpresaIndustria}"><i class="fa fa-edit"></i></div>`;
